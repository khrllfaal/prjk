#!/usr/bin/env python3
"""
Replace ONLY the kas_masuk (cash in) rows in `transactions` with a
fresh Cash In Report file, leaving kas_keluar/bank_masuk/bank_keluar
...
Usage:
    ...
    python3 scripts/update_kas_masuk_mysql.py --dry-run cash_in_Report.xls
"""
import argparse
import os
import re
...
def main():
    ...
    if jenis != 'kas_masuk':
        print(f"ERROR: file ini terdeteksi sebagai '{jenis}', bukan kas_masuk.", file=sys.stderr)
        sys.exit(1)

    projects_referenced = sorted({t['project'] for t in txns if t['project']})
    total_debet = sum(t['debet'] for t in txns)
    print(f"Kas Masuk baru: {len(txns)} baris, total {total_debet:,.0f}")
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("DELETE FROM transactions WHERE jenis='kas_masuk'")
            deleted = cur.rowcount
            print(f"\n{deleted} baris kas_masuk lama dihapus (kas_keluar/bank_masuk/bank_keluar tidak disentuh).")
            ...
            cur.executemany(
                ...
                rows)
        conn.commit()
        print(f"\nUpdate selesai: {len(txns)} baris kas_masuk baru dimuat, sudah di-commit ke database.")
    except Exception:
        conn.rollback()
        raise
    finally:
