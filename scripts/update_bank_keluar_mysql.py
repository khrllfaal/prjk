#!/usr/bin/env python3
"""
Replace ONLY the bank_keluar (bank out) rows in `transactions` with a
fresh Bank Out Report file, leaving kas_masuk/kas_keluar/bank_masuk
...
Usage:
    ...
    python3 scripts/update_bank_keluar_mysql.py --dry-run bank_out_Report.xls
"""
import argparse
import os
import re
...
def main():
    ...
    ap.add_argument('file', help='bank out report .xls (HTML)')
    ap.add_argument('--dry-run', action='store_true')
    args = ap.parse_args()

    jenis, txns = parse_report(args.file)
    if jenis != 'bank_keluar':
        print(f"ERROR: file ini terdeteksi sebagai '{jenis}', bukan bank_keluar.", file=sys.stderr)
        sys.exit(1)

    projects_referenced = sorted({t['project'] for t in txns if t['project']})
    total_kredit = sum(t['kredit'] for t in txns)
    print(f"Bank Keluar baru: {len(txns)} baris, total {total_kredit:,.0f}")
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("DELETE FROM transactions WHERE jenis='bank_keluar'")
            deleted = cur.rowcount
            print(f"\n{deleted} baris bank_keluar lama dihapus (kas_masuk/kas_keluar/bank_masuk tidak disentuh).")
            ...
            cur.executemany(
                ...
                rows)
        conn.commit()
        print(f"\nUpdate selesai: {len(txns)} baris bank_keluar baru dimuat, sudah di-commit ke database.")
    except Exception:
        conn.rollback()
        raise
    finally:
