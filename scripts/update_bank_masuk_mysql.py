#!/usr/bin/env python3
"""
Replace ONLY the bank_masuk (bank in) rows in `transactions` with a
fresh Bank In Report file, leaving kas_masuk/kas_keluar/bank_keluar
...
Usage:
    ...
    python3 scripts/update_bank_masuk_mysql.py --dry-run bank_in_Report.xls
"""
import argparse
import os
import re
...
def main():
    ...
    ap.add_argument('file', help='bank in report .xls (HTML)')
    ap.add_argument('--dry-run', action='store_true')
    args = ap.parse_args()

    jenis, txns = parse_report(args.file)
    if jenis != 'bank_masuk':
        print(f"ERROR: file ini terdeteksi sebagai '{jenis}', bukan bank_masuk.", file=sys.stderr)
        sys.exit(1)

    projects_referenced = sorted({t['project'] for t in txns if t['project']})
    total_debet = sum(t['debet'] for t in txns)
    print(f"Bank Masuk baru: {len(txns)} baris, total {total_debet:,.0f}")
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("DELETE FROM transactions WHERE jenis='bank_masuk'")
            deleted = cur.rowcount
            print(f"\n{deleted} baris bank_masuk lama dihapus (kas_masuk/kas_keluar/bank_keluar tidak disentuh).")
            ...
            cur.executemany(
                ...
                rows)
        conn.commit()
        print(f"\nUpdate selesai: {len(txns)} baris bank_masuk baru dimuat, sudah di-commit ke database.")
    except Exception:
        conn.rollback()
        raise
    finally:
